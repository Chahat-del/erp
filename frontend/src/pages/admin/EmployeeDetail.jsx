...
export default function EmployeeDetail() {
  ...
  const perfData = perf ? [
    { name: 'Completion Rate', value: perf.performance.completionRate, fill: '#3b82f6' }
  ] : [];
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="card mb-6">
        <div className="flex items-start gap-6">
          <div className={`w-20 h-20 rounded-2xl flex items-center justify-center text-white text-2xl font-bold ${avatarColor(employee.name)}`}>
            {getInitials(employee.name)}
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-2xl font-bold text-gray-900">{employee.name}</h1>
              <span className="font-mono text-blue-600 bg-blue-50 px-2 py-0.5 rounded text-sm">{employee.employeeId}</span>
              {employee.isActive ? <span className="badge-green">Active</span> : <span className="badge-red">Inactive</span>}
            </div>
            <p className="text-gray-500">{employee.designation} {employee.department && `· ${employee.department}`}</p>
            <div className="flex flex-wrap gap-4 mt-3 text-sm text-gray-500">
              ...
              {employee.address && <span className="flex items-center gap-1"><MapPin className="w-4 h-4" />{employee.address}</span>}
              <span className="flex items-center gap-1"><Calendar className="w-4 h-4" />Joined {formatDate(employee.joiningDate)}</span>
            </div>
      ...
      {perf && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="card text-center">
            <p className="text-3xl font-bold text-blue-600">{perf.performance.completionRate}%</p>
            <p className="text-sm text-gray-500 mt-1">Task Completion Rate</p>
          </div>
          <div className="card text-center">
            <p className="text-3xl font-bold text-green-600">{perf.performance.completedOnTime}</p>
            <p className="text-sm text-gray-500 mt-1">Completed On Time</p>
          </div>
          <div className="card text-center">
            <p className="text-3xl font-bold text-purple-600">{perf.performance.totalReports}</p>
            <p className="text-sm text-gray-500 mt-1">Total Reports</p>
          </div>
          <div className="card text-center">
            <p className="text-3xl font-bold text-orange-600">{perf.performance.totalHoursWorked}h</p>
        ...
        <div className="card mb-6">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={perf.hoursPerWeek}>
              ...
              <XAxis dataKey="_id.week" tick={{ fontSize: 12 }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <h3 className="font-semibold text-gray-900 mb-4">Assigned Tasks ({tasks.length})</h3>
        ...
        <div className="card">
          <h3 className="font-semibold text-gray-900 mb-4">Recent Reports ({reports.length})</h3>
          <div className="space-y-2 max-h-72 overflow-y-auto">
            ...
            {reports.map(r => (
              <div key={r._id} className="py-2 border-b border-gray-50 last:border-0">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-gray-900">{r.project?.name}</p>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-xs text-gray-400">{r.hoursWorked}h worked</span>
                  {r.adminReview?.reviewed ? <span className="badge-green text-xs">Reviewed</span> : <span className="badge-yellow text-xs">Pending</span>}
                </div>
              </div>
            ))}
          </div>
